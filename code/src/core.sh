    ...
    if [[ $is_use_tls ]]; then
        if [[ ! $is_no_auto_tls && ! $is_caddy && ! $is_gen ]]; then
            ...
            [[ $(is_test port_used 80) || $(is_test port_used 443) ]] && {
                ...
                msg "\e[41m no-auto-tls 帮助(help)\e[0m: $(msg_ul https://ebj.cc/$is_core/no-auto-tls/)\n"
...
get() {
    ...
    protocol)
        ...
        *socks*)
            is_protocol=socks
            net=socks
            [[ ! $is_socks_user ]] && is_socks_user=xkd333
...
info() {
    ...
    tcp | kcp | quic)
        ...
        is_vmess_url=$(jq -c '{v:2,ps:'\"xkd333-${net}-$is_addr\"',add:'\"$is_addr\"',port:'\"$port\"',id:'\"$uuid\"',aid:"0",net:'\"$net\"',type:'\"$header_type\"',path:'\"$kcp_seed\"'}' <<<{})
    ...
    ss)
        ...
        is_url="ss://$(echo -n ${ss_method}:${ss_password} | base64 -w 0)@${is_addr}:${port}#xkd333-$net-${is_addr}"
    ...
    ws | h2 | grpc)
        ...
        [[ $is_protocol == 'vmess' ]] && {
            is_vmess_url=$(jq -c '{v:2,ps:'\"xkd333-$net-$host\"',add:'\"$is_addr\"',port:'\"$is_https_port\"',id:'\"$uuid\"',aid:"0",net:'\"$net\"',host:'\"$host\"',path:'\"$path\"',tls:'\"tls\"'}' <<<{})
            is_url=vmess://$(echo -n $is_vmess_url | base64 -w 0)
        } || {
            [[ $is_trojan ]] && {
            ...
            is_url="$is_protocol://$uuid@$host:$is_https_port?encryption=none&security=tls&type=$net&host=$host&${is_url_path}=$(sed 's#/#%2F#g' <<<$path)#xkd333-$net-$host"
        }
    ...
    reality)
        ...
        is_url="$is_protocol://$uuid@$is_addr:$port?encryption=none&security=reality&flow=xtls-rprx-vision&type=tcp&sni=$is_servername&pbk=$is_public_key&fp=ios#xkd333-$net-$is_addr"
        ;;
    door)
    ...
    socks)
        ...
        is_url="socks://$(echo -n ${is_socks_user}:${is_socks_pass} | base64 -w 0)@${is_addr}:${port}#xkd333-$net-${is_addr}"
        ;;
    http)
        is_can_change=(0 1)
        is_info_show=(0 1 2)
    ...
    if [[ $is_new_install ]]; then
        warn "首次安装请查看脚本帮助文档: $(msg_ul https://ebj.cc/$is_core/$is_core-script/)"
    fi
    if [[ $is_url ]]; then
        msg "------------- ${info_list[12]} -------------"
    ...
    if [[ $is_no_auto_tls ]]; then
        ...
        msg "\e[41m帮助(help)\e[0m: $(msg_ul https://ebj.cc/$is_core/no-auto-tls/)"
    fi
    footer_msg
}
...
url_qr() {
    ...
    if [[ $is_url ]]; then
        [[ $1 == 'url' ]] && {
            ...
            link="https://xkd333.github.io/tools/qr.html#${is_url}"
...
is_main_menu() {
    ...
    msg "群组 (Chat): $(msg_ul https://t.me/tgxkd333)"
    is_main_start=1
    ask mainmenu
    case $REPLY in
    1)
